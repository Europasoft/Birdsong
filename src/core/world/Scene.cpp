...
namespace WorldSystem
{
	...
	void Scene::initGlobalDescriptorSet()
	{
		...
		currentCamera = std::make_shared<EngineCore::Camera>(CameraSettings{ .fieldOfViewDeg = 85, .nearDistance = 1.0, .farDistance = 8 * 1000 * 100 });
		currentCamera->transform.rotation = { 0.f, 0.f, 0.f };
		currentCamera->transform.translation = { -22222.f * 1000.f * 100.f, 0.f, 0.f }; // 22222 km away is a good distance to test eart-sized planet at center

		// scene global descriptors
		UBO_Struct ubo1{};
		ubo1.add(uelem::mat4); // MVP matrix
		ubo1.add(uelem::vec4); // viewport resolution and full swapchain resolution
		...
		instanceBuffer = InstanceBufferUtil::allocate<EngineCore::InstanceData>(device, 1000000);
	...
	void Scene::updateDescriptors(uint32_t frameIndex, double deltaTime)
	{
		...
		sceneGlobalDescriptorSet->writeUBOMember(0, res, EngineCore::UBO_Layout::ElementAccessor{ 1, 0, 0 }, frameIndex);
	}
